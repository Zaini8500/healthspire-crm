...
export default function Notes({ leadId }: { leadId?: string }) {
  ...
  const deleteCategory = async (id: string) => {
    ...
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/note-categories/${id}`, { method: "DELETE" });
      const json = await res.json().catch(() => null);
  ...
  const deleteLabel = async (id: string) => {
    ...
    if (!ok) return;
    try {
      const res = await fetch(`${API_BASE}/api/note-labels/${id}`, { method: "DELETE" });
      const json = await res.json().catch(() => null);
      if (!res.ok) throw new Error(json?.error || "Failed");
          ...
          <Tabs value={tab} onValueChange={setTab}>
            ...
            <TabsContent value="list">
              <div className="flex items-center gap-2 flex-wrap mb-3">
                ...
                <Button type="button" variant="success" size="icon" aria-label="refresh" onClick={loadNotes}><RefreshCw className="w-4 h-4"/></Button>
